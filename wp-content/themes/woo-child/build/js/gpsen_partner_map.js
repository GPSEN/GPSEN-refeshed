		...
		var initialize = function initialize() {
			...
			for (add = 0; add < json.length; add++) {
				...
				google.maps.event.addListener(marker, 'click', function (marker, add) {
					return function () {
						infoWindow.setContent('<img src="' + json[add]._embedded['wp:featuredmedia'][0].source_url + '">' + '<span>' + '<p><strong>' + json[add].title.rendered + '</strong></p>' + '<p><strong>' + json[add]['post-meta-fields'].Address[0] + '</strong></p>' + '<p>Website: ' + '<a href="' + json[add]['post-meta-fields'].Website + '" target="_blank">' + json[add]['post-meta-fields'].Website + '</a>' + '</p>' + '</span>' + '<div class="clearBoth"></div>');
